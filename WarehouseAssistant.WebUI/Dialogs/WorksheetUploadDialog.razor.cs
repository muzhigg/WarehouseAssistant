...
{
    public partial class WorksheetUploadDialog<TTableItem> : ComponentBase, IDisposable
        where TTableItem : class, ITableItem, new()
    {
        [Parameter] public DynamicExcelColumn[]? ExcelColumns { get; set; }
        ...
        private void MatchColumns()
        {
            ...
            foreach (var excelColumn in ExcelColumns)
            {
                ...
                if (columnMapping.TryGetValue(propAttribute.Name, out var columnLetter) ||
                    (propAttribute.Aliases != null &&
                     propAttribute.Aliases.Any(alias => columnMapping.TryGetValue(alias, out columnLetter))))
